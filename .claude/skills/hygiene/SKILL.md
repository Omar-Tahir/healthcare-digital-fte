...
Search for hardcoded secrets in two places: Python source files and the `.env`
file. The risk is real: a single committed credential means a rotation incident,
and in a healthcare project that can also mean HIPAA exposure.

**1a — Scan Python source files:**
...
Unsafe pattern: `api_key = "sk-ant-..."` — always flag as CRITICAL.

**1b — Scan `.env` for live key patterns:**
...
if [ -f .env ]; then
  grep -n \
    ...
    -e "AKIA" \
    -e "xoxb-" \
    .env 2>/dev/null | grep -v "your_key_here\|placeholder\|example\|changeme"
  echo ".env scanned"
else
...
Any real-looking key value (not a placeholder): FLAG as CRITICAL.

**1c — Confirm `.env` is gitignored:**

```bash
grep -q "^\.env" .gitignore 2>/dev/null \
  && echo ".env is gitignored — OK" \
  || echo "WARNING: .env not in .gitignore — add it now"
```

A live key in an un-gitignored `.env` is the most common credential leak vector.
Both 1b and 1c must be CLEAN before marking this check green.
